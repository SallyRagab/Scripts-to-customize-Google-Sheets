...
function respondToOK(e) {
  ...
  var result1 = e.parameter.tBox1.replace(/ /g,"-").replace(/\$/g,"").replace(/\+/g,"").toLowerCase();
  var result2 = e.parameter.tBox2.replace(/ /g,"-").replace(/\$/g,"").replace(/\+/g,"").toLowerCase();
...
//DDM convert
function getInput() {
  var s2 = SpreadsheetApp.getActiveSpreadsheet();
  //Create the user interface
  var app2 = UiApp.createApplication()
    .setWidth(300)
    .setHeight(100);
  //Create one textboxs
  var tBox12 = app.createTextBox().setName('tBox1').setId('tBox1');
  //Create the OK button and a handler to respond to when OK is clicked.
  var okHandler2 = app.createServerHandler('respondToOK');
  var btnOK2 = app.createButton('OK', okHandler)
      .setWidth(60)
      .setHeight(20);;
  //I find grids very handy for arranging labels and widgets like textboxes neatly
  var myGrid2 = app.createGrid(3, 2).setId('myGrid2')
    .setText(0, 0, 'HO ID:')
    .setWidget(0, 1, tBox1)
    .setWidget(2, 1, btnOK)
    .setWidth(300)
  ...
  /*The OK button's handler needs a callback element otherwise it can't read the user's input.
    ...
    As all the textboxes are added to myGrid then just one callback to that will work fine.
  */
  okHandler.addCallbackElement(myGrid2);
  app.add(myGrid2);
  s2.show(app2);
}
...
function respondTo(e) {
  ...
  var result = e.parameter.tBox1.replace(/ /g,";").toLowerCase();
  
  //Enter the result in the cell
  var sh = SpreadsheetApp.getActiveSpreadsheet().getSheets()[0];
  var cellAddress4 = sh.getActiveCell().getA1Notation();
  sh.getRange("D5").setValue(result);
